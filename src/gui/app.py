...
def create_gui():
    ...
    def on_check_connection():
        ...
        info_text.insert(
            ...
            f"🤖 안드로이드 버전: {device_info['platformVersion']}\n"
    ...
    log_message("   • 스팸 전화번호 및 차단 단어 자동 추가 프로그램")
    log_message("   • 최대 등록 한도 팝업 확인용 (스크립트별 일정 시간 소요)")
    log_message("   • 실행 전 APPIUM 환경 설정이 필요합니다.")
    log_message("=" * 60)
    log_message("📋 사용방법:")
    log_message("   1. 디바이스 연결 확인")
    log_message("   2. 스크립트 선택")
    log_message("      ❗ 스크립트 실행 전, 연결한 단말에서 해당 앱에서 실행할 기능 화면으로 진입해주세요.")
    log_message("      예) 익시오 스팸번호 추가 - 설정 > 스팸 알림 및 수신 차단 > 전화 차단 진입")
    log_message("   3. 스크립트 실행")
    log_message("=" * 60)
